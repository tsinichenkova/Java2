...
import ru.geekbrains.qa.java2.lesson7.entity.WeatherResponse;
    ...
    private void getWeatherForFiveDays (String cityKey) throws IOException {
        ...
        for (JsonNode dailyForecast : dailyForecasts) {
            System.out.printf("В городе %s на дату %s ожидается %s, минимальная температура - %s %s, максимальная температура - %s %s\n",
                    ApplicationGlobalState.getInstance().getSelectedCity(),
                    StringUtils.substringBefore(dailyForecast.at("/Date").asText(), "T"),
                    dailyForecast.at("/Day/IconPhrase").asText().toLowerCase(Locale.ROOT),
                    dailyForecast.at("/Temperature/Minimum/Value").asText(),
                    dailyForecast.at("/Temperature/Minimum/Unit").asText(),
                    dailyForecast.at("/Temperature/Maximum/Value").asText(),
                    dailyForecast.at("/Temperature/Maximum/Unit").asText());
